"""
Estimate a generalized linear model with high dimensional categorical variables / instrumental variables
...
function reg(df::AbstractDataFrame, f::Formula;
    ...
    if isa(vcov, Symbol)
        ...
        vcovformula = GLFixedEffectModels.VcovFormula(Val{vcov.args[1]}, (vcov.args[i] for i in 2:length(vcov.args))...)
    end
    ...
    ##############################################################################
    rf = deepcopy(f)
    (has_iv, iv_formula, iv_terms, endo_formula, endo_terms) = GLFixedEffectModels.decompose_iv!(rf)
    rt = GLFixedEffectModels.Terms(rf)
    ...
    vars = GLFixedEffectModels.allvars(rf)
    iv_vars = GLFixedEffectModels.allvars(iv_formula)
    endo_vars = GLFixedEffectModels.allvars(endo_formula)
    absorb_vars = GLFixedEffectModels.allvars(feformula)
    vcov_vars = GLFixedEffectModels.allvars(vcovformula)
    ...
    if has_absorb
        fes, ids = GLFixedEffectModels.parse_fixedeffect(df, GLFixedEffectModels.Terms(@eval(GLFixedEffectModels.@formula(nothing ~ $(feformula)))))
        if drop_singletons
            for fe in fes
                GLFixedEffectModels.drop_singletons!(esample, fe)
            end
        end
    end
    ...
    # Compute weights
    sqrtw = GLFixedEffectModels.get_weights(df, esample, weights)
    ...
    if any([isa(fe.interaction, GLFixedEffectModels.Ones) for fe in fes])
        rt.intercept = false
        has_intercept = true
    end
    fes = GLFixedEffectModels.FixedEffect[GLFixedEffectModels._subset(fe, esample) for fe in fes]
    

    # Compute data for std errors
    vcov_method_data = GLFixedEffectModels.VcovMethod(df[esample, unique(Symbol.(vcov_vars))], vcovformula)
    ...
    mf = GLFixedEffectModels.ModelFrame2(rt, df, esample; contrasts = contrasts)

    # Obtain y
    # for a Vector{Float64}, conver(Vector{Float64}, y) aliases y
    y = convert(Vector{Float64}, GLFixedEffectModels.model_response(mf)[:])
    yname = rt.eterms[1]
    y .= y .* sqrtw

    # Obtain X
    coef_names = GLFixedEffectModels.coefnames(mf)
    has_exo = !isempty(mf.terms.terms) | mf.terms.intercept
    if has_exo
        Xexo = GLFixedEffectModels.ModelMatrix(mf).m
        Xexo .= Xexo .* sqrtw
    else
    ...
    tss = GLFixedEffectModels.compute_tss(y, has_intercept, sqrtw)

    if !has_absorb
        error("We're assuming you have fixed effects here.")
    end

    # some constants
    coeflength = length(coef_names) # TODO this should really be better
    beta = zeros(Float64, coeflength)

    # choose better starting values
    
    eta = Xexo * beta + linkfun.(Ref(link),0.1.*ones(nobs))
    mu = linkinv.(Ref(link),eta)
...
#   dev <- sum(family[["dev.resids"]](y, mu, wt))
    ...
    for i=1:maxiter
        ...
        # mu.eta <- family[["mu.eta"]](eta) # THIS IS d mu / d eta
        # w.tilde <- sqrt(mu.eta^2 / family[["variance"]](mu))
        # nu <- as.vector((y - mu) / mu.eta)  # THIS IS NU^r in (2.5)
        mymueta = mueta.(Ref(link),eta)
        wtilde = sqrt.(mymueta.*mymueta ./  glmvar.(Ref(distribution),mu) )
        nu = (y - mu) ./ mymueta

        # # Pseudo-demean variables
        # M <- pseudo.demeaning(cbind(nu, X) * w.tilde, w.tilde, A, B, lvls.k, control[["pseudo.tol"]])
        pfe = GLFixedEffectModels.FixedEffectMatrix(fes, wtilde, Val{method})
        # initialize iterations and converged
        iterations = Int[]
        convergeds = Bool[]
        nudemean, b, c = FixedEffects.solve_residuals!(nu, pfe; maxiter = maxiter, tol = tol)
        append!(iterations, b)
        append!(convergeds, c)

        Xdemean, b, c = FixedEffects.solve_residuals!(Xexo, pfe; maxiter = maxiter, tol = tol)
        append!(iterations, b)
        append!(convergeds, c)
        ...
        eta_update = nu - (nudemean - Xdemean * beta_update) ./ wtilde
        ...
        rho = 1.0
        while true
            mu = linkinv.(Ref(link),eta + rho .* eta_update)
            dev = sum(devresid.(Ref(distribution), y, mu))
            println("dev = $(dev)")
        ...
        if ((devold - dev)/dev < dev_tol) 
            println("Iter $i : converged (deviance)")
            break
        end
        if (norm(beta_update) < step_tol )
            println("Iter $i : converged (step size)")
            break
        end

        println("Iter $i : not converged")
        println("---------------------------------")
    end
